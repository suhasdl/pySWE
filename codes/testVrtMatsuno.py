...
Unlike traditional Mtsuno setup (which uses mass source/sink), here the system is forced with a vorticity source and sink pair.
This leads to a robust super-rotating state.
(See Suhas D. L., Jai Sukhatme, and Joy M. Monteiro. “Tropical vorticity forcingand superrotation in the spherical shallow-water equations.” Quarterly Journal of the Royal Meteorological Society 143, no. 703 (2017): 957-965.)
"""
...
########### Vorticity forcing #########################################
# pair of  gaussian vorticity blobs are used to force the system
# change it to any other config if so desired
lats_ref   = np.radians(0.)
lons_ref1  = np.radians(90.)
lons_ref2  = np.radians(270.)
delta_lats = np.radians(10.)
delta_lons = np.radians(30.)
vrtamp = 5e-11
S1 = vrtamp * np.exp(-((lons-lons_ref1)/(delta_lons))**2) * np.exp(-((lats-lats_ref)/(delta_lats))**2)
S2 = vrtamp * np.exp(-((lons-lons_ref2)/(delta_lons))**2) * np.exp(-((lats-lats_ref)/(delta_lats))**2)
S  = S1-S2
Hbar = 300. # mean height

# Initialise other fields
zero_array = np.zeros_like(S)
htsp = x.grdtospec(zero_array + Hbar) # ht is initalised to Hbar
...
def dfdt(t,fn,args=None):
    ...
    dvrtsp = -tmpb + x.grdtospec(S - vrt/tau_vrt)
...
filename = 'vrtMatsuno.nc'              # file name
...
initial_name = ['vrtFr'] # inital fields, these are saved only once
initial_data = [S]
param_name = ['radius', 'rotation', 'gravity', 'tau_vrt', 'tau_div', 'tau_ht', 'Hbar', 'vrtamp', 'time_step'] # constants which a single value rather than a 2D field
param_data = [rsphere, omega, grav, tau_vrt, tau_div, tau_ht, Hbar, vrtamp, dt]
...
while(t<tmax):
        ...
        if np.mod(ii,200) ==0:
            ...
            plt.figure(1)
            plt.clf()
            plt.contourf(lonDeg,latDeg,vrt,16,extend='both',cmap='coolwarm')
